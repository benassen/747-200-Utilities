...
if (AIRCRAFT_FILENAME == "B742_PW_Felis_XP12.acf" ) then
----------------------------------------------------------------------------------------------------
-- Missing datarefs
...
icl=icl+1
cl_descent = icl
check_lists[icl]             = { ['code'] = 'DESCENT' ,  ['title'] = 'Descending ' , ['active']=true} 
  ...
  if ( cl == 'FE-INIT') then
   ...
   elseif ( cl == 'OVERHEAD-CHK') then 
      ...
      if (perf_ind1 ~= nil and  desired_pi1 ~= nil and perf_ind1 <= desired_pi1  ) then
        ...
        add_schedule (timer_2, 'body_gr_steer_sw', 'disarm' ) ;
   ...
   elseif ( cl == 'DESCENT') then 
      ------------------------------------------------------ 
        current_cl_num = cl_num
	add_schedule (-1, 'TITLE', 'Descent') ;	
	add_schedule (timer_2, 'cabin_alt_rotary', 21 )

	-- Open aux crossfead valves
	add_schedule (0, 'MSG', 'CROSSFEED VALVES') ; 
        add_schedule (0, 'fuel_crossfeed_valves', 'open', 5 ) ;
	add_schedule (0, 'fuel_crossfeed_valves', 'open', 6 ) ;
   ...
   elseif ( cl == 'DESCENT-10000') then 
      ------------------------------------------------------ 
        ...
        -- turn off fuel heat
        add_schedule (0, 'MSG', 'FUEL HEAT OFF') ;
        add_schedule (timer_2, 'fuel_heat_sw', 'off' , 1 ) ;
        add_schedule (timer_2, 'fuel_heat_sw', 'off' , 2 ) ;
        add_schedule (timer_2, 'fuel_heat_sw', 'off' , 3 ) ;
        add_schedule (timer_2, 'fuel_heat_sw', 'off' , 4 ) ; 

	-- Close aux crossfead valves
	add_schedule (0, 'MSG', 'CROSSFEED VALVES') ; 
        add_schedule (0, 'fuel_crossfeed_valves', 'close', 5 ) ;
	add_schedule (0, 'fuel_crossfeed_valves', 'close', 6 ) ;
     ...
     elseif ( cl == 'TAXI-AFTER-LDG') then 
      ------------------------------------------------------ 
        ...
        add_schedule (timer_2, 'pressu_mode', 'man' ) 
	...
	-- turn off ignition
     ...
     elseif ( cl == 'PARKING') then 
      ------------------------------------------------------ 
        ...
        -- Turn off beacon
	add_schedule (0, 'MSG', 'BEACON OFF') ;
        add_schedule (timer_2, 'beacon', 'off' )
        ...
        add_schedule (timer_1, 'probe_heater_R', 'off') ;
        add_schedule (0, 'MSG', 'WINDOW HEATERS') ;
        add_schedule (timer_1, 'window_heat_sw_2L', 'off') ; 
        add_schedule (timer_1, 'window_heat_sw_1L', 'off') ;
        add_schedule (timer_1, 'window_heat_sw_1R', 'off') ;
        add_schedule (timer_1, 'window_heat_sw_2R', 'off') ;
